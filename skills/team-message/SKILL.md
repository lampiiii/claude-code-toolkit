...
## Voice & Tone (Based on Real Examples)

The output should sound like Abhay writing to a colleague — natural, respectful, and clear.

### How Abhay Actually Writes:
...
**User input**: "Jeremy needs to know the TPSM adoption index sheet has been updated with December data"
...
After saving the message, you MUST automatically open the file in Cursor so the user can copy from a formatted view:

```bash
cursor /Users/abhay.tyagi/Desktop/AI/pm-workspace/conversations/teams/teams-conversations.md
```

This opens the file in Cursor. The user can then hit `Cmd+Shift+V` to toggle markdown preview for formatted copy-paste.
