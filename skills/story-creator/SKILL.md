...
- `Enable Now Assist prompt for success plays`
...
* Create a new business rule on the `csm_account_health` table that triggers when the `license_utilization` field crosses 90%.
* The business rule fires an event to the `csm_trigger_events` table with event_type = "license_threshold_breach", severity, and account reference.
* Add a lookup from `csm_trigger_events` to the `csm_play_mapping` decision table to resolve the recommended play based on event_type + account_segment.
* Store the recommendation in the `csm_play_recommendations` table with fields: account, recommended_play, trigger_event, status (pending/accepted/dismissed), and created_timestamp.
...
* Add ACL rules to restrict read/write access on `csm_play_recommendations` to the `csm_manager` and `csm_admin` roles.
```
