...
**The final deliverable must be a Word document (.docx).** Use Python (e.g., `python-docx`) to generate the document programmatically.

**Typography — applies to the entire document:**
- **Font**: Verdana throughout (no other fonts)
- **Subsection headings** (e.g., Problem Statement / Overview, Persona(s), Functional Details, etc.): **Verdana 12pt**
- **Body text** under each subsection: **Verdana 10pt**
- **Epic title (Subject)**: Verdana 12pt, bold

Generate the epic content first, then render it as a .docx with these formatting rules.
...
- Integration points (APIs, Now Assist, external systems) if applicable
...
- Now Assist prompt invocation count and completion rate
...
11. **Always output a Word document (.docx).** Use Verdana throughout — 12pt for subsection headings, 10pt for body text. Save the .docx to the same `workings/` directory as the feature.
...
- [ ] Output is a Word document (.docx) with Verdana font — 12pt headings, 10pt body
